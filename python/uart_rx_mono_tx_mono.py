...
wf = wave.open(get_file_path('output', 'song_input.wav'), 'rb')
...
def LoopbackTest(comPortName):
    ...
    try:
        while is_any_thread_alive([transmit_thread,receive_thread, play_audio_thread]):
            time.sleep(0)
        ...
        if timeout_error:
            ...
            wavfile.write(get_file_path('output', 'song_output.wav'), fs, np.asarray(recieved_data).astype(np.int16))
